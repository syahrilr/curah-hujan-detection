...
export async function syncPompaData() {
  try {
    ...
    // --- NORMALISASI DATA (UPDATE LOGIKA DISINI) ---
    ...
    const listTMA = (Array.isArray(rawTMA) ? rawTMA : rawTMA.data || []).map((item: any) => {
      // UPDATE: Field 'TANGGAL' di screenshot sudah berisi format lengkap "YYYY-MM-DDTHH:mm:ss"
      let sourceTime = item.TANGGAL || item.tanggal;

      // Fallback untuk format lama (jika ada API lain yang pisah tanggal & jam)
      ...
      return {
        ...
        waktu_source: sourceTime // Simpan waktu dari screenshot
      };
    });
        ...
        operationsCH.push({
          insertOne: {
            document: {
              ...
              waktu_sensor: nearestCH.waktu_source // Simpan data TANGGAL_TERAKHIR_HARI_INI
            }
          }
        });
      }
        ...
        operationsTMA.push({
          insertOne: {
            document: {
              ...
              fetched_at: now,
              waktu_fetch_wib: wibString,
              waktu_sensor: nearestTMA.waktu_source // Simpan data TANGGAL
            }
          }
        });
      }
