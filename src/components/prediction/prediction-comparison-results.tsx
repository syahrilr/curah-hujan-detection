...
import { exportComparisonToCSV, exportComparisonToJSON, prepareComparisonChartData } from '@/lib/comparison-export-utils';
import {
    Activity,
    AlertTriangle,
    BarChart3,
...
export function PredictionComparisonResults({ data }: ComparisonResultsProps) {
  ...
  const accuracyPercentage = statistics.totalLocations > 0
    ? (statistics.accurateCount / statistics.matchedLocations) * 100
    : 0;
        ...
        <CardHeader>
          <div className="flex justify-between items-center">
            <div>
              ...
              <CardDescription>
                Prediction accuracy analysis for {statistics.totalLocations} locations
              </CardDescription>
            </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {statistics.averageError.toFixed(2)}
            </div>
            <p className="text-xs text-muted-foreground">
              {statistics.averageErrorPercentage.toFixed(1)}% relatif
            </p>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold text-blue-500">
              {statistics.matchedLocations}
            </div>
            <p className="text-xs text-muted-foreground">
              dari {statistics.totalLocations} lokasi
            </p>
          </CardContent>
        </Card>
            ...
            <div className="space-y-1">
              <div className="flex justify-between text-xs">
                ...
                <span className="font-bold text-green-500">
                  {statistics.qualityDistribution.excellent}
                </span>
              </div>
              <div className="flex justify-between text-xs">
                <span>Good:</span>
                <span className="font-bold text-blue-500">
                  {statistics.qualityDistribution.good}
                </span>
              </div>
            </div>
          </CardContent>
        ...
        <TabsContent value="details">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Prediksi vs Data Real (± {data.toleranceMinutes} menit)
              </CardDescription>
            </CardHeader>
            <CardContent>
                            ...
                            <div className="space-y-1">
                              ...
                              <div className="space-y-1">
                                ...
                                <div className="flex justify-between">
                                  ...
                                  <span className="text-xs">
                                    {comp.actual.timeDiffMinutes.toFixed(1)} min
                                  </span>
                                </div>
                              </div>
                            </div>
